...
func benchmarkRoutes(b *testing.B, router *echo.Echo, routes []*Route) {
	b.ReportAllocs()
...
func loadFiberRoutes(fb *fiber.App, routes []*Route) {
	for _, r := range routes {
		switch r.Method {
		case "GET":
			fb.Get(r.Path, fiberHandler(r.Method, r.Path))
		case "POST":
			fb.Post(r.Path, fiberHandler(r.Method, r.Path))
		case "PATCH":
			fb.Patch(r.Path, fiberHandler(r.Method, r.Path))
		case "PUT":
			fb.Put(r.Path, fiberHandler(r.Method, r.Path))
		case "DELETE":
			fb.Delete(r.Path, fiberHandler(r.Method, r.Path))
		}
	}
}

func fiberHandler(method, path string) fiber.Handler {
...
func BenchmarkFiberStatic(b *testing.B) {
	fb := fiber.New()
	loadFiberRoutes(fb, static)
	benchmarkRoutesFb(b, fb, static)
}

func BenchmarkFiberGitHubAPI(b *testing.B) {
	fb := fiber.New()
	loadFiberRoutes(fb, githubAPI)
	benchmarkRoutesFb(b, fb, githubAPI)
}

func BenchmarkFiberGplusAPI(b *testing.B) {
	fb := fiber.New()
	loadFiberRoutes(fb, gplusAPI)
	benchmarkRoutesFb(b, fb, gplusAPI)
}

func BenchmarkFiberParseAPI(b *testing.B) {
	fb := fiber.New()
	loadFiberRoutes(fb, parseAPI)
	benchmarkRoutesFb(b, fb, parseAPI)
}


func benchmarkRoutesFb(b *testing.B, router *fiber.App, routes []*Route) {
	b.ReportAllocs()
	r := httptest.NewRequest("GET", "/", nil)
	u := r.URL
	//w := httptest.NewRecorder()
